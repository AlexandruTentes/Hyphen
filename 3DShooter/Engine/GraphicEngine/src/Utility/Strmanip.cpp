#include "../../lib/Utility/Strmanip.h"
...
std::string string_manipulation::read(std::string str, void(*handler)(std::string &),
	short unsigned int to_output, short unsigned int get_word, char by_char1, char by_char2, char ignore_char)
{
	std::string output = "";
	std::ifstream fout;
	std::string word = "";
	...
	try
	{
		...
		while (fout.good())
		{
			...
			if (!ignore && get_word && (c == by_char1 || c == by_char2 || c == '\n'))
			{
				if (print)
				{
					handler(word);
			...
			{
				word += (!ignore && get_word) * c;
				print = (!ignore && get_word) * print;
			}
